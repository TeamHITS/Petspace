...
class OrderController extends AppBaseController
{
    ...
    public function show($id)
    {
        ...
        return view('admin.orders.show')->with(['order' => $order, 'petspace' => $petspace, 'title' => $this->BreadCrumbName, 'transactions' => $transactions, 'usercards' => $usercards]);
    }
    ...
    public function makePayment(Request $request)
    {
        ...
        if ($this->updateOrderServicesAddon($request)) {
            ...
            if ($card_status == 'oldcard') {
                ...
                if (isset($response['order'])) {
                    ...
                    $subject = "Petspace Order Payment Link";
                    $email   = $order->user->email;
                    $name    = $userdetail->first_name . ' ' . $userdetail->last_name;

                    Mail::send('email.paymentlink', ['name' => $userdetail->first_name, 'payment_link' => $response['order']['url']],
		...
		if($card_status == 'oldcard'){
            ...
            return view('admin.orders.payment_status')->with(['order' => $order, 'response' => $response, 'title' => $this->BreadCrumbName, 'card_status' => $status]);
        }
    }
