...
use App\Criteria\PackageCriteria;
...
class PackageAPIController extends AppBaseController
{
     ...
     *     //@SWG\Parameter(
     *          name="petspace_id",
     *          description="Pass the petspace id you want to get packages. If not found, Returns All Records in DB",
     *          type="integer",
     *          required=false,
     *          in="query"
     *      ),
    ...
    {
        $packages = $this->packageRepository->resetCriteria()
            ->pushCriteria(new RequestCriteria($request))
            ->pushCriteria(new LimitOffsetCriteria($request))
            ->pushCriteria(new packageCriteria([
                'petspace_id'             => $request->get('petspace_id', null),
            ]))
            ->all();
    ...
    public function show($id)
    {
        ...
        if (empty($package)) {
            return $this->sendErrorWithData(['Package not found']);
        }
