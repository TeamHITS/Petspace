...
class AuthAPIController extends AppBaseController
{
     ...
     * )
     */
    public function getForgetPasswordCode(Request $request)
    {
        $user = $this->userRepository->getUserByEmail($request->email);
        ...
        try {
            ...
            if ($request->type == 10 || $request->type == 30) {
...
//            if($request->type == 20 || $request->type == 30 ){
//                $smsClient = new SmsController();
...
//            DB::table('password_resets')->insert(['email' => $email, 'code' => $code, 'created_at' => Carbon::now()]);
            ...
            $smsClient = new SmsController();
            $smsClient->sendVerificationSms($phone, $code);

        } catch (\Exception $e) {
            return $this->sendErrorWithData([$e->getMessage()], 403);
        }
        return $this->sendResponse([], 'Verification Code Send To Your Number');
    }
     ...
     * )
     */
    public function getNewPhoneVerificationCode(Request $request)
    {
        $user = $this->userRepository->getUserByEmail($request->email);
...
//            DB::table('password_resets')->where('code', $check->email)->delete();
     ...
     *      summary="verify phone verification request code.",
     *      tags={"Passwords"},
     *      description="verify code",
     *      produces={"application/json"},
     *     @SWG\Parameter(
     ...
     * )
     */
    public function verifyPhoneCode(VerifyCodeRequest $request)//
    {
        ...
        $smsClient = new SmsController();
        $status    = $smsClient->verifyCode($phone, $code);

        if ($status == "approved") {
            $data = $this->userRepository->getUserByEmail($email);
            DB::table('user_details')->where('user_id', $data->id)->update(['is_verified' => 1]);
            $data = $this->userRepository->getUserByEmail($email);
     ...
     * )
     */
    public function verifyNewPhoneCode(Request $request)
    {
    ...
    public function checkEmail(Request $request)
    {
        ...
        return $this->sendResponse($user, 'Email address found.');
    }
    ...
    public function getServerTime()
    {
        ...
        return $this->sendResponse($data, 'Server time fetched.');
    }
}
