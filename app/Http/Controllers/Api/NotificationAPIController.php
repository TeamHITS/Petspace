...
class NotificationAPIController extends AppBaseController
{
     ...
     *      summary="send pushnotification to user.",
     *      tags={"Notification"},
     ...
     *          description="Notification that should be stored",
     *          required=false,
     *          @SWG\Schema(ref="#/definitions/Push-Notification")
     *      ),
     *      @SWG\Response(
     *          response=200,
     *          description="successful operation",
     ...
     *              @SWG\Property(
     *                  property="data",
     *                  ref="#/definitions/Push-Notification"
     *              ),
     *              @SWG\Property(
     *                  property="message",
     *                  type="string"
     ...
     * )
     */
    public function sendNotification(CreateNotificationAPIRequest $request)
    {
        $input = $request->all();

        $devices = app('App\Repositories\Admin\UDeviceRepository')->findWhere(['user_id' => $input['reciever_id']]);
        if (count($devices) > 0) {
            $notificationsHelper = new NotificationsHelper();
            $notificationsHelper->sendPushNotifications($input['message'], $devices, ['action_type' => $input['action_type'], 'ref_id' => $input['ref_id']]);
        }

        return $this->sendResponse($input, 'Notification send successfully');
    }
}
