...
@section('content')
                    ...
                    <div class="service-menu-card-wrap">
                        ...
                        @foreach($submenus as $submenu)
                            <div class="service-menu-card sub-menu-card-3">
                                <div class="text">
                                    <p class="title">{{$submenu['name']}}<a class="btn-edit-submenu"
                                                                            data-id="{{$submenu['id']}}"
                                ...
                                @if($submenu['condition_option'] != null)
                                    <div class="bottom">
                                        <p>•Optional Items
                                            • {{$submenu['condition_text'] }} {{$submenu['select_count'] }} Item</p>
                                    </div>
                                @endif
                            </div>
                            @foreach($submenu['service'] as $submenuService)
                                <div class="service-menu-card service-menu-card-3 sub-menu-card-2">
                                    <div class="top">
                                        <div class="text">
                                            <p class="name">{{$submenuService['name']}}<a
                                                        class="btn-edit-submenu-service"
                            ...
                            <div class="service-menu-card add-new-service">
                                <a href="#!" class="add-btn btn-add-submenu-service" data-id="{{$submenu['id']}}"
                                   style="cursor: pointer;"><i class="fas fa-plus mr-1"></i>New Service</a>
                            </div>
                        @endforeach
                        <div class="service-menu-card add-service-category">
                            <a href="#!" class="add-btn btn-add-submenu" style="cursor: pointer;"><i
                                        class="fas fa-plus mr-1"></i>NEW SUBMENU LIST</a>
                        </div>
                    </div>
                </div>
            </div>
    ...
    <div class="modal fade gen-modal technician-modal" id="editNewList" tabindex="-1" aria-hidden="true">
    </div>

    <div class="modal fade gen-modal technician-modal" id="editSubMenuNewService" tabindex="-1" aria-hidden="true">
    </div>
@endsection
@push('scripts')
    ...
    <script>
        ...
        $('body').on('change', '#submenu-condition-chk', function () {
            if (!$(this).is(':checked')) {
        ...
        $(".btn-add-submenu").click(function () {

            var url = "{{url('admin/petspaces/add-submenu-modal').'/'.$service['id']}}";
            ajaxGet(url, "", (status, data) => {
                if (status) {

                    $("#addNewList").html(data.data);
                    // cloneRow();
        ...
        $(".btn-add-submenu-service").click(function () {

            var catId = $(this).data('id');
            var url = "{{url('admin/petspaces/add-submenu-service-modal').'/'}}" + catId;
            ajaxGet(url, "", (status, data) => {
                if (status) {

                    $("#addSubMenuNewService").html(data.data);
                    // cloneRow();
                    $('#addSubMenuNewService').modal('show');
                }
            });

        });

        $(".btn-edit-submenu").click(function () {

            var catId = $(this).data('id');
            // $('#passenger-added-alert').css('display', 'none');
            var url = "{{url('admin/petspaces/edit-submenu-modal').'/'}}" + catId;
            ajaxGet(url, "", (status, data) => {
                if (status) {

                    $("#editNewList").html(data.data);
                    $('#editNewList').modal('show');
                }
            });

        });

        $(".btn-edit-submenu-service").click(function () {

            var catId = $(this).data('id');
            // $('#passenger-added-alert').css('display', 'none');
            var url = "{{url('admin/petspaces/edit-submenu-service-modal').'/'}}" + catId;
            ajaxGet(url, "", (status, data) => {
                if (status) {
                    $("#editSubMenuNewService").html(data.data);
                    $('#editSubMenuNewService').modal('show');
                }
            });
