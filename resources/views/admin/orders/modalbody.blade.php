    ...
    <div class="panel-heading">
      <h4 class="panel-title">
        <a class="accordion-toggle" data-toggle="collapse" data-parent="#accordion2" href="#{{$catserv->id}}">
          ...
          $discountedPrice = $catserv['price'] - $catserv['price']*($catserv['discount']/100);

          @endphp
          <h4 class="product-name"><strong>{{$catserv['name']}}</strong> <strong style="float:right">AED {{$discountedPrice}}</strong></h4>
      ...
      <input type="hidden" name="service_price" value="{{$discountedPrice}}" />
        ...
        <?php foreach($catserv['submenu'] as $service_submenu){?>
          ...
          <div class="panel-body {{$service_submenu['name']=='Addons' ? 'd-none' : ''}}">
              ...
              foreach($service_submenu['service'] as $service){
                ...
                $discountedAddonPrice = $service['price'] - $service['price']*($service['discount']/100);
  
                if($service_submenu['name']!='Addons') { ?>
                  
                <label class="radio">
                  <input type="radio" name="petsize" value="{{$service['name']}}" data-price="{{$discountedAddonPrice}}" data-id="{{$service['id']}}" data-duration="{{$service['service_duration']}}" id="{{$service['name']}}">
                <h4 class="product-name">
                  <strong>{{$service['name']}} </strong><strong style="float:right"> AED {{$discountedAddonPrice}}</strong></h4>
                ...
                <label class="checkbox">
                  <input type="checkbox" name="addons[]" data-name="{{$service['name']}}" data-price="{{$discountedAddonPrice}}" value="{{$service['id']}}" id="addon_{{$service['id']}}" />
                  <h4 class="product-name">
                    <strong>{{$service['name']}} </strong><strong style="float:right">AED  {{$discountedAddonPrice}}</strong></h4>
                  <h5> <small>{{$service['description']}}</small> </h5>
                </label> 
